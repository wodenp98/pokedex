...
export async function getEvolutionOfPokemon(url: string) {
  ...
  if (data.id === 18) {
    ...
    const gloomEvolution = currentStage.evolves_to[0];
    const gloomPokemon = await getPokemonData(gloomEvolution.species.name);
    const gloomPokemonSpecies = await getInformationsForPokemon(
      gloomPokemon.id
    );
    const gloomPokemonFrenchName = await getFrenchName(gloomPokemonSpecies);

    evolutionChain.push({
      ...gloomPokemon,
      frenchName: gloomPokemonFrenchName.name,
      evolves: gloomEvolution.evolution_details[0],
    });

    const finalEvolutions = [];

    for (const evolution of gloomEvolution.evolves_to) {
      const finalEvolutionPokemon = await getPokemonData(
        evolution.species.name
      );
      const finalEvolutionPokemonSpecies = await getInformationsForPokemon(
        finalEvolutionPokemon.id
      );
      const finalEvolutionPokemonFrenchName = await getFrenchName(
        finalEvolutionPokemonSpecies
      );

      finalEvolutions.push({
        ...finalEvolutionPokemon,
        frenchName: finalEvolutionPokemonFrenchName.name,
        evolves: evolution.evolution_details[0],
      });
    }

    evolutionChain.push(finalEvolutions);
  } else if (data.id === 67) {
    let currentStage = data.chain;
